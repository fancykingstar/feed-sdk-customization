...
DEFAULT_DOWNLOAD_LOCATION = path.expanduser('./feed-sdk')
...
class Feed(object):
    ...
    def __auth_token(self):
        ...
        auth_headers = {
            ...
            "Authorization": "Basic UGVhcmNlQXMtYWVkMC00N2VkLTg3NzEtYWE0ZTE4MmE2OGQ3OjljNzQwNzBiLWQ3ZjktNDI0Yy05ZjE3LTk2MDRhYjlkNDVmMg=="
        }

        auth_feed_response = requests.post(auth_endpoint, data=auth_parameters, headers=auth_headers)
        response = json.loads(auth_feed_response._content)
    ...
    def __invoke_request(self, file_handler):
        ...
        while status_code == 206:
            ...
            #     if file_handler_:
            #         file_handler_.close()
            #     file_name = f'output-1-{count // 5}.gz'
            #     file_handler_ = open(file_name, 'wb')

            count = count + 1
            print(count)
            if count % 100 == 0:
                token = self.__auth_token()
                headers[const.AUTHORIZATION_HEADER] = 'Bearer ' + token
            # Append the data to the file, might raise an exception
            file_utils.append_response_to_file(file_handler, feed_response.data)
